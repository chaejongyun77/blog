...
// blog에 최상단 제목과 이미지 날짜 카테고리를 표시하는 부분 - 애니메이션 추가
const postcategoryStyle = `category-badge bg-activation text-primary text-sm font-medium px-3 py-1.5 rounded-lg tracking-wide transition duration-200 ease-in-out hover:bg-blue-400 hover:text-white cursor-pointer`;
const posttitleStyle = `md:text-[40px] md:leading-[56px] text-[32px] leading-[40px] font-bold my-3 fade-in-up`;

const postauthordateDivStyle = `md:mb-8 mb-6 h-fit`;
const postauthorDivStyle = `inline-block`;
const postauthorImgStyle = `inline w-8 h-8 rounded-full object-cover object-center mr-2 border border-graylv2 overflow-hidden transition-transform duration-200 hover:scale-110`;
...
const postimgtitleStyle = `w-full max-h-[520px] object-cover object-center my-4 rounded-2xl mx-auto block max-w-full align-middle shadow-lg hover:shadow-2xl transition-shadow duration-300`;
const postsectionStyle = `w-full mb-10 md:mb-[60px] max-w-full h-auto align-middle fade-in-up`;
...
// bloglist 목록 스타일 - 애니메이션 강화
const bloglistFirstCardStyle = `blog-card lg:col-span-3 md:col-span-2 col-span-1 h-auto rounded-2xl overflow-hidden bg-white flex md:flex-row flex-col flex-1 md:mb-[20px] cursor-pointer fade-in-up`;
const bloglistFirstCardImgStyle = `blog-card-img w-full object-cover object-center rounded-2xl overflow-hidden md:h-auto h-[200px] md:w-[49%] lg:w-[52%] shrink-0 mr-8`;
...
const bloglistCardStyle = `blog-card lg:max-w-sm overflow-hidden bg-white rounded-2xl cursor-pointer col-span-1 w-auto fade-in-up`;
const bloglistCardImgStyle = `blog-card-img w-full h-[200px] object-cover object-center rounded-2xl overflow-hidden`;

const bloglistCardBodyStyle = `py-4 px-2`;
const bloglistCardTitleStyle = `font-bold text-2xl mb-3 hover:text-primary transition-colors duration-200`;
const bloglistCardCategoryStyle = `category-badge inline-block bg-activation text-primary md:text-sm font-medium mb-3 px-3 py-1.5 rounded-lg transition duration-200 ease-in-out hover:bg-blue-400 hover:text-white`;
...
const bloglistCardAuthorImgStyle = `inline w-8 h-8 rounded-full object-cover object-center mr-2 border border-graylv2 overflow-hidden transition-transform duration-200 hover:scale-110`;
...
// paginationStyle - 애니메이션 강화
const paginationStyle = `mt-20 mb-[132px] flex justify-center items-center gap-8`;
const pageMoveButtonStyle = `relative flex inline-flex items-center rounded-[10px] p-[11px] text-graylv2 hover:text-graylv4 bg-graylv1 transition-all duration-200 hover:bg-gray-200`;
const pageNumberListStyle = `flex items-center justify-center gap-1`;
const pageNumberStyle = `page-number relative inline-flex items-center w-10 h-10 px-4 py-2 text-md font-normal text-graylv3 cursor-pointer`;
