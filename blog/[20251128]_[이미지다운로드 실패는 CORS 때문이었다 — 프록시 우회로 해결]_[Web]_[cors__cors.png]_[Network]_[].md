...
html2canvas와 toBlob()을 사용해 DOM 전체를 캡처하는 기능을 구현했다.
기능 자체는 복잡하지 않았지만 실제로 다운로드를 실행했을 때 이미지 생성이 되지 않는 문제가 발생했다.

브라우저 개발자 도구에서 로그를 확인해보니
여러 이미지 리소스 로딩 시 CORS origin 오류가 발생하고 있었다.
DOM 캡처 과정에서 일부 이미지가 정상적으로 로딩되지 않아 캔버스가 생성되지 않은 것이다.

이 포스팅에서는 해당 문제가 왜 발생했는지
그리고 어떻게 해결했는지 정리해보려 한다.

# CORS Origin은 무엇인가?
<strong>CORS(Cross-Origin Resource Sharing)</strong>는
브라우저가 보안상의 이유로 다른 출처(origin) 에 있는 리소스에 접근할 때
그 요청을 허용할지 판단하는 규칙이다.
...
그 리소스 서버가 명시적으로 접근을 허용하지 않으면 요청을 차단한다.
...
하지만 문제는 제가 관리할 수 없는 <strong>외부 도메인</strong>의 이미지였다.
해당 서버의 CORS 정책에 접근할 수 없기 때문에
어떠한 설정도 적용할 수 없었다.

이 경우 html2canvas는 계속해서 오류를 발생시켰다.
...
4.  프론트는 이를 “우리 서버에서 내려온 이미지”로 사용한다.

이 방식에서는 브라우저가 보기에  이미지가 “외부가 아닌 우리 서버에서 온 것”으로 간주된다.
결과적으로 캔버스가 오염되지 않고 이미지 다운로드가 정상적으로 작동한다.
...
# 마무리하며
html2canvas는 매우 편리한 라이브러리지만,
브라우저의 CORS 정책과 맞물리면 예상치 못한 제약이 생길 수 있다.

이번 문제는 CDN 리소스는 서버 헤더 수정으로 해결 가능했지만 외부 사이트 리소스는 해당 서버의 정책을 바꿀 수 없기 때문에 백엔드 프록시를 통해 동일 출처로 변환하는 방식으로 해결했다.

결과적으로 외부 이미지가 포함된 페이지라도 문제없이 DOM 전체를 이미지로 변환할 수 있게 되었다.
