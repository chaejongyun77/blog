...
어드민 페이지에서 엑셀 다운로드 기능을 공통으로 구현하던 중, 테스트 과정에서 심각한 문제를 발견했습니다. 서버의 CPU 사용률이 급격히 증가하면서 최종적으로 **OOM(Out Of Memory)** 에러가 발생하여 서버가 먹통이 되었습니다. 따라서 이번 포스팅에서는 이 문제를 어떻게 분석하고 해결했는지 공유하고자 합니다.
...
XSSFWorkbook은 Excel 2007 이상의 XLSX 파일 형식을 처리하는 라이브러리입니다. 하지만 다음과 같은 단점이 있습니다

1. **메모리 처리 방식**: 전체 워크북 데이터를 메모리에 올려서 처리합니다.
...
따라서 30만 건의 데이터를 처리할 경우 각 행과 셀에 대한 객체가 모두 메모리에 생성되어 수 GB의 메모리를 소비하게 됩니다.
...
```java
 SXSSFWorkbook(10000).use { workbook ->
            val sheet = workbook.createSheet("sheet1")

            val headerStyles = createHeaderStyles(workbook)
            val tableStyles = createTableStyles(workbook)

            var rowIdx = 0

            for (headerRow in header.rows) {
                val row = sheet.createRow(rowIdx++)
                var colIdx = 0

                for (cellInfo in headerRow.cells) {
                    val cell = row.createCell(colIdx)
                    cell.setCellValue(cellInfo.text)

                    cell.cellStyle = headerStyles[cellInfo.style] 
                            ?: headerStyles[ExcelHeaderStyle.DEFAULT]!!

                    val span = cellInfo.colSpan.coerceAtLeast(1)
                    if (span > 1) {
                        sheet.addMergedRegion(
                            CellRangeAddress(
                                row.rowNum, row.rowNum,
                                colIdx, colIdx + span - 1
                            )
                        )
                    }
                    colIdx += span
            ...
            val colHeaderRow = sheet.createRow(rowIdx++)
            table.columns.forEachIndexed { col, name ->
                val cell = colHeaderRow.createCell(col)
                cell.setCellValue(name)
                cell.cellStyle = tableStyles[TableStyleKey.COLUMN]!!
            }
            ...
            table.rows.forEach { values ->
                ...
                values.forEachIndexed { col, v ->
                    ...
                    setCellValue(cell, v)
                    cell.cellStyle = tableStyles[TableStyleKey.BODY]!!
                }
            }

            val headerCols = header.rows.firstOrNull()?.cells?.sumOf { it.colSpan.coerceAtLeast(1) } ?: 0
            val totalCols = maxOf(headerCols, table.columns.size)
            
            for (c in 0 until totalCols) {
                sheet.setColumnWidth(c, 20 * 256)
            }

            val fileName = buildFileName(request, "result")
            ...
            // SXSSFWorkbook는 임시 파일을 생성하므로 명시적으로 정리
            workbook.dispose()
        }
    }
```

- SXSSFWorkbook(10000) : 적절한 윈도우 크기를 설정하는것도 중요하며 최근 10,000행만 메모리에 유지
- workbook.write(outputStream) : 이 시점에 메모리 + 임시파일을 합쳐서 .xlsx를 만들어서 출력
- workbook.dispose() : SXSSF가 만든 임시파일을 삭제하여 자원 회수
...
#### 1. XSSFWorkbook 사용 시
![XSSFWorkbook 메모리 사용량](./img/excel/excel.png)
- **Heap 최대 사용량**: 약 4GB
- **처리 시간**: 20~30초 이상

<br>

#### 2. SXSSFWorkbook 사용 시
![XSSFWorkbook 메모리 사용량](./img/excel/excel2.png)
- **Heap 최대 사용량**: 약 400MB
- **처리 시간**: 7초
