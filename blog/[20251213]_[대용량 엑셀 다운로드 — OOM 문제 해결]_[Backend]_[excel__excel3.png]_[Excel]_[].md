...
어드민 페이지에서 엑셀 다운로드 기능을 공통으로 구현하던 중, 테스트 과정에서 심각한 문제를 발견했습니다. 
서버의 CPU 사용률이 급격히 증가하면서 최종적으로 **OOM(Out Of Memory)** 에러가 발생한 것입니다.
이번 포스팅에서는 이 문제를 어떻게 분석하고 해결했는지 공유하고자 합니다.
