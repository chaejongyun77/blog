...
“WebSocket 연결은 특정 WAS 인스턴스에 종속되기 때문에
하나의 WebSocket 연결을 통해
여러 WAS 서버로 동시에 메시지를 전달할 수는 없다”

>WebSocket만으로 클라이언트가 WAS1, WAS2에 동시에 연결할 수 있나?

👉 Client ── WebSocket ── WAS-1
로드밸런서를 거쳐도 연결이 맺어지는 순간 하나의 WAS에 고정

하나의 WebSocket 연결이 WAS-1에도 보내고 WAS-2에도 보내고 이런 구조는 WebSocket 자체로는 안 됨

>WebSocket은 클라이언트와 서버 간의 실시간 양방향 통신을 가능하게 하지만,
연결이 특정 WAS 인스턴스에 종속된다는 특징이 있다.

<br>

#### 결국 서버가 여러 대일 때 문제가 생긴다
이 때문에 WAS 서버가 여러 대로 구성된 환경에서는 한 서버에서 발생한 WebSocket 메시지를
다른 서버가 직접 전달받을 수 없다.

> 이러한 한계를 해결하기 위해
서버 간 메시지를 공유할 수 있는 Kafka를 도입했고,
모든 WAS 서버가 동일한 실시간 이벤트를 처리할 수 있도록 구성했다.
...
우선 카프카를 이해하기 위해서는 이벤트 브로커와 메세지 큐에 대한 이해가 필요하다.
...
전송속도가 빠르고 전송 결과를 신속하게 알 수 있는 장점이 있는 반면에, 특정 개체에 장애가 발생한 경우 메세지를 보내는 쪽에서 대기 처리 등을 개별적으로 해주지 않으면 장애가 전파될 수 있다.    
또한 참여하는 개체가 많아질 수록 각 개체를 연결해줘야 한다. (=> 시스템이 커질수록 확장성이 좋지 않아진다)
...
토픽별로 설정을 다르게 가져갈 수 있기 때문에 파티션을 기능별로 분리하였다.
예를들어 실시간으로 사용자의 커서위치를 나타내는 기능은 파티션을 늘려 병렬로 처리하여 지연 최소화를 우선적으로 생각하였다.   
그러나 UndoRedo 기능같은 경우는 컨슈머 동시성을 1로 설정하는게 안전하게 순서를 보장할 수 있기때문에    

또한 기능별로 토픽을 나누면 모니터링도 기능별로 한눈에 파악할 수 있다.
[사진첨부]
이렇게 lag과 처리량등 기능단위로 추적하기 쉬워져서 장애 대응에 빨라질 수 있다.
...
![
kafka
](./img/kafka/kafka12.png)

컨슈머는 파티션을 병렬로 처리하기 때문에 댓글 작성 → 좋아요 클릭 → 게시물 생성과 같이 순서가 뒤죽박죽 섞여서 실행되는 치명적인 오류가 생길 수 있다.

> 그래서 Key를 쓰면 어떻게 되는건대?

Kafka에서 같은 key를 가진 메시지는 항상 같은 파티션으로 갑니다. (partition = hash(key) % 파티션 수)
예를들어 key가 "123"이면 항상 partition 1로 가고 key가 "456"이면 항상 항상 partition 2로 갑니다.

여기서 우리 서비스에서 중요한건 보드 내부에서 발생한 이벤트들의 순서가 중요하다고 생각했기 때문에 boardId를 key로 설정하였습니다.
따라서 같은 boardId는 항상 같은 파티션으로 메시지가 가고 **결과적으로는 보드 단위로 순서가 보장되는 실시간 이벤트 처리가 가능해집니다.**


#### STEP4. groupId 설정

단일 WAS 환경에서는 문제가 없었지만 WAS가 was1, was2 두 대 이상으로 늘어나면서 이상한 현상이 발생했다

선생님(사용자1)이 게시글을 작성하면
사용자2(학생)는 실시간으로 보임 ✅
사용자3(학생)는 실시간으로 안 보임 ❌

> 원인: WAS들이 같은 Consumer Group으로 동작하고 있었음

 Kafka에서 **consumer group은 “메시지를 나눠서 처리하는 단위”**다.
 같은 groupId를 사용하는 컨슈머들은 토픽의 메시지를 서로 나눠서 소비한다

해결: WAS마다 서로 다른 groupId 사용

그래서 구조를 이렇게 바꿨다.

was1 → groupId = live-was1

was2 → groupId = live-was2

이렇게 하면:

Kafka 입장에서는

was1, was2를 서로 다른 consumer group으로 인식
...
자신에게 연결된 WebSocket 클라이언트들에게 브로드캐스트

👉 실시간 메시지가 모든 사용자에게 정상적으로 전달된다
