...
기존 시스템에서는 **세션 + JWT Access Token** 조합을 사용해 로그인 상태를 유지하고 있었습니다.

Access Token을 통해 사용자를 식별하며 세션을 함께 사용하면서 서버 측에서도 로그인 상태를 관리하는 구조였습니다.

하지만 최근 여러 기업에서 발생한 개인정보 유출 사고와
인증·인가 취약점 이슈를 계기로 로그인 보안 구조 전반을 다시 점검할 필요가 있다고 판단했습니다.

특히 세션 기반 상태 관리와 만료 제어가 어려운 Access Token 단독 사용 방식은 보안과 확장성 측면에서 한계가 명확했습니다.

이러한 문제를 개선하기 위해 기존 세션 구조를 제거하고 **JWT Access Token + Refresh Token + Redis** 기반의 로그인 로직으로 구조를 개선하였습니다.

***
<br>

# JWT토큰이란 무엇인가?
**JWT(Json Web Token)** 은 Json 객체에 인증에 필요한 정보들을 담은 후 **비밀키로 서명한 토큰**으로 인터넷 표준 인증 방식입니다. 
...
Access Token의 한계를 인지한 후 가장 먼저 적용한 개선 방법은 Refresh Token을 도입하는 것이었습니다.

<br>

#### Refresh Token이란?
Refresh Token은 Access Token이 만료되었을 때 새로운 **Access Token을 재발급받기 위해 사용하는 토큰**입니다.
...
위와 같은 방식은 구현이 단순하고 서버에 별다른 저장소가 필요 없다는 장점이 있습니다.
...
Refresh Token과 Redis를 도입하면서 Access Token 단독 사용 구조의 한계는 상당 부분 개선되었습니다.

하지만 JWT토큰을 활용한 로그인 구조를 설계하면서 느낀점은 여전히 아래와같은 문제점이 생길 수 있다는 생각을 하게 되었습니다.


#### 1. Local Storage는 안전한가? ####
> <span style="color:red"> Access Token의 유효시간이 아무리 짧더라도 해킹당하게 되면 그 시간동안은 보안이 위험한거 아닌가..?    

현재 구조에서는 아래와 같이 저장하고 있습니다.
...
Access Token의 유효시간을 짧게 설정했기 때문에 탈취되더라도 공격 가능 시간은 제한됩니다.
하지만 근본적인 질문은 여전히 남습니다.
> Local Storage에 저장된 토큰은 정말 안전한가?

Local Storage는 다음과 같은 특성을 가집니다.
...
즉, Access Token의 유효시간이 아무리 짧더라도 XSS가 발생하면 그 시간 동안은 토큰 탈취가 가능합니다.
...
Refresh Token을 Redis에 저장함으로써 토큰을 서버에서 통제할 수 있게 되었지만 동시에 Redis에 대한 의존성도 생겼습니다.

- 네트워크 장애
- Redis 장애
- TTL 만료 처리 실패

위와 같은 오류가 생긴다면 로그인을 처리할 때 Refresh Token이 Redis에 저장이 안되어 오류가 생길 것이고 또한 Access Token이 만료된 사용자는 Refresh Token 검증이 불가능해지게 될 것입니다.
...
현재 운영 중인 서비스는 무료이지만 향후 유료 서비스로 전환되고 결제 시스템이 도입된다면
로그인 및 인증 보안의 중요성은 더욱 커질 수밖에 없습니다.

이러한 배경에서 기존 JWT 기반 로그인 구조를 점검하고
Refresh Token과 Redis를 활용한 방식으로 인증 구조를 개선하게 되었습니다.
