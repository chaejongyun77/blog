...
- Post 1 -> DB row 5개
- Post 2 -> DB row 50개
2) Post 10개를 기준으로 페이지 1을 만들어달라고 요청함

3) 이때 fetch join +페이징 이슈가 발생함
만약 DB가 LIMIT 10을 적용하면 DB에서는 row를 기준으로 10줄만 잘라서 반환함
따라서
```
P1-O1
P1-O2
...
```
총 row 10개를 반환함
4) 하지만 JPA가 이걸 엔티티(Post)로 묶으면?
JPA는 Post기준으로 묶으니까
- Post 1 -> row 5개
- Post 2 -> row 5개
결과적으로는 Post는 단 2개만 생성됩니다. 즉 개발자가 기대한 페이징결과는 Post가 10개가 반환되는건데 실제 fech join을 해서 페이징으로 가져오면 Post는 2개만 가져오게 됩니다.
5) 결론적으로는 문제 발생
