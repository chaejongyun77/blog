...
특히 연관관계(@OneToMany, @ManyToOne 등)에 기본으로 설정되어 있는 지연로딩(LAZY) 은 N+1 문제의 가장 대표적인 원인입니다.
...
> 애초에 JOIN으로 한 번에 가져오면 N+1 문제도 안 생기고 더 좋은 것 아닌가?
...
조회하면  1 X 10 X 5 X 3 = 150개의 row를 조회하게 됩니다.
...
N+1 문제의 원인은 연관 엔티티를 지연로딩으로 하나씩 추가적으로 조회하는것 때문이라면 가장 먼저 떠올릴 수 있는 해결책은 fetchJoin을 사용하는 것입니다.
#### fetch join 이란?
...
- Post 1 -> DB row 5개
- Post 2 -> DB row 50개
Post 10개를 기준으로 페이지 1을 만들어달라고 요청 -> 이때 fetch join +페이징 이슈가 발생
만약 DB가 LIMIT 10을 적용하면 DB에서는 row를 기준으로 10줄만 잘라서 반환합니다. 따라서  
<br>
...
코드가 길어지기도 하고 다른 개발자가 처음 이 코드를 봤을때도 복잡해보일 수 있다는 단점이 있습니다.
