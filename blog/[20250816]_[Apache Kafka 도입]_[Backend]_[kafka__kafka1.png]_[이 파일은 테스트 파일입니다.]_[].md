...
Kafka는 같은 파티션 안에서만 순서를 보장하는 분산 메시징 시스템이고 속도를 내기 위해 하나의 토픽을 여러 개의 파티션으로 나누고 각 파티션을 병렬로 처리합니다
...
단일 WAS 환경에서는 문제가 없었지만 WAS 서버가 was1, was2 두 대 이상으로 늘어나면서 이상한 현상이 발생했습니다
...
### 마무리하며
Kafka를 도입하면서 전자칠판 서비스의 실시간 이벤트 처리를
단순한 WebSocket 전송이 아닌
확장성과 안정성을 고려한 메시징 구조로 재설계할 수 있었다.

기능 단위 토픽 분리, 파티션 기반 병렬 처리
그리고 Consumer Group 설계를 통해
멀티 WAS 환경에서도 실시간 이벤트를 안정적으로 전달하고 있다.
...
또한 Kafka 운용 시 중요한 부분 중 하나는 Lag이 얼마나 쌓이는지 항상 모니터링하는 것입니다.   

Lag란 Kafka에 미처리된 이벤트가 얼마나 남았는지에 대한 메트릭인데 테스트 과정에서 적정한 Consumer 개수와 Partition 개수를 정할 때 Lag을 보면서 설계해야 합니다.
