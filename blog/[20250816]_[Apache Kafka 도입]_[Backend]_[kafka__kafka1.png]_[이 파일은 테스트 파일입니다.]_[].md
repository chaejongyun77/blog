...
**토픽을 기능별로 분리**한거는 여러가지 이유가 있는데 그 중 하나는 장애/지연 현상을 감소하기 위해서다.
...
lag와 처리량등을 기능단위로 추적하기 쉬워져서 운영 및 모니터링이 쉬워져 빠른 대응이 가능합니다.
...
Kafka에서 병렬 처리의 핵심은 파티션과 컨슈머의 동시성(Concurrency)이다.   

저의 서비스에서는 각 토픽에 대해 파티션을 여러 개로 구성하고  
@KafkaListener에서 concurrency = "3" 설정을 통해 하나의 WAS 인스턴스에서도 동시에 여러 메시지를 처리하도록 설계했다.
...
Kafka는 같은 파티션 안에서만 순서를 보장하는 분산 메시징 시스템이고 속도를 내기 위해 하나의 토픽을 여러 개의 파티션으로 나누고 각 파티션을 병렬로 처리한다.
