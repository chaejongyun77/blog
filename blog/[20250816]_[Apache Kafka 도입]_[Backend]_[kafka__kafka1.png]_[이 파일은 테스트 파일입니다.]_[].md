...
이 글에서는 실제 서비스에서 이벤트가 어떤 흐름으로 전달되는지를 중심으로 **전자칠판 서비스 내 실시간 메시징 처리**가 어떻게 이루어지고 있는지 정리해보려 합니다.
...
TCP를 기반으로 한 웹 소켓은 신뢰성 있는 데이터 전송을 보장하며 순서가 보장된 양방향 통신을 제공할 수 있습니다. HTTP와 다르게 클라이언트와 서버 간에 최초 연결이 이루어지면, 이 연결을 통해 양방향 통신을 지속적으로 할 수 있습니다.   
...
**WebSocket이 실시간 메시징의 핵심 역할**을 한다.
...
따라서 하나의 WebSocket 연결이 WAS-1에도 보내고 WAS-2에도 보내고 이런 구조는 WebSocket 만으로는 안됩니다.
...
따라서 이러한 한계를 해결하기 위해 **서버 간 메시지를 공유할 수 있는 Kafka를 도입했고 모든 WAS 서버가 동일한 실시간 이벤트를 처리할 수 있도록 구성**했습니다.
...
카프카(Kafka)는 파이프라인, 스트리밍 분석, 데이터 통합 및 미션 크리티컬 애플리케이션을 위해 설계된 **고성능 분산 이벤트 스트리밍 플랫폼**이다. 또한 Pub-Sub 모델의 메시지 큐 형태로 동작하며 분산환경에 특화되어 있습니다.
...
5. 보장성: MQ에 들어간다면 결국 모든 메시지가 consumer 서비스에게 전달 보장
...
이러한 형태의 단점을 극복하고자 나온게 **Pub/Sub 모델**입니다.

![
kafka
](./img/kafka/kafka5.png)

Pub/Sub 모델은 비동기 메세징 전송 방식으로 발신자의 메세지에는 수신자가 정해져 있지 않은 상태로 publish 합니다. 그리고 이를 구독을 한 수신자만 정해진 메세지를 받을 수 있습니다.
...
**토픽을 기능별로 분리**한거는 여러가지 이유가 있는데 그 중 하나는 **장애/지연 현상을 감소**하기 위해서입니다.
예를들어 한 기능(board)에서 메시지가 기하급수적으로 늘어나도 토픽을 분리했기 때문에 다른 기능에는 영향이 가지 않아 어느정도 오류를 방지할 수 있습니다.

또한 **토픽별로 설정값을 다르게 가져갈 수 있어서 튜닝하기에 용이**합니다.
만약 board에 관한 이벤트가 제일 많다고 판단되면 그에 해당하는 토픽만 파티션이나 컨슈머를 늘리는 설정을 가져갈 수 있습니다.

그리고 **토픽이 분리되어 있으면 모니터링도 기능별로 한눈에 파악**할 수 있습니다.
따라서 lag와 처리량등을 기능단위로 추적하기 쉬워져서 운영 및 모니터링이 쉬워져 빠른 대응이 가능합니다.

![
kafka
](./img/kafka/kafka13.png)

<br>

### 2. 파티션과 동시성(Concurrency) 설정

Kafka에서 병렬 처리의 핵심은 파티션과 컨슈머의 동시성(Concurrency)입니다.  

저의 서비스에서는 각 토픽에 대해 파티션을 여러 개로 구성하고 @KafkaListener에서 concurrency = "3" 설정을 통해 하나의 WAS 인스턴스에서도 동시에 여러 메시지를 처리하도록 설계했습니다.

![
kafka
](./img/kafka/kafka14.png)


#### 파티션과 Concurrency의 관계

- 파티션 : 하나의 파티션은 동시에 하나의 컨슈머 스레드만 처리 가능
- Concurrency : 하나의 @KafkaListener가 내부적으로 생성하는 컨슈머 스레드 개수
- 만약 concurrency가 3이면 하나의 WAS에서 최대 3개의 컨슈머가 병렬로 poll

> 실제 병렬 처리 수 = 파티션 수 × 컨슈머 수 (WAS 수 기준)

**결론적으로는 단일 컨슈머로 메시지를 처리하면 이벤트가 몰릴 경우 하나의 메시지가 끝날 때까지 대기를 하게 되는데
concurrency 설정을 통해 병렬 처리를 하여 메시지 처리 지연을 줄일 수 있습니다.**
...
Kafka는 같은 **파티션 안에서만 순서를 보장하는 분산 메시징 시스템**이고 속도를 내기 위해 하나의 토픽을 여러 개의 파티션으로 나누고 각 파티션을 병렬로 처리합니다
...
컨슈머는 파티션을 병렬로 처리하기 때문에 댓글 작성 → 좋아요 클릭 → 게시물 생성과 같이 순서가 뒤죽박죽 섞여서 실행되는 치명적인 오류가 생길 수 있습니다.

<br>

> 그래서 Key를 쓰면 어떻게 되는건대?

Kafka에서 같은 key를 가진 메시지는 항상 같은 파티션으로 갑니다.
예를들어 key가 "123"이면 항상 partition 1로 가고 key가 "456"이면 항상 항상 partition 2로 갑니다.
...
단일 WAS 환경에서는 문제가 없었지만 **WAS 서버가 was1, was2 두 대 이상으로 늘어나면서 이상한 현상이 발생**했습니다
선생님(사용자1)이 게시글을 작성하면 사용자2(학생)는 실시간으로 보이는데 사용자3(학생)는 실시간으로 안 보이는 경우가 생겼습니다.

원인을 파악한 결과 WAS 서버들이 같은 Consumer Group으로 동작하고 있어서 메시지를 was1 또는 was2 중 한 곳만 소비하고 있는 현상이 문제였습니다.

참고로 Kafka에서 **consumer group은 “메시지를 나눠서 처리하는 단위”**입니다. 따라서 같은 groupId를 사용하는 컨슈머들은 토픽의 메시지를 서로 나눠서 소비합니다.

>WAS마다 서로 다른 groupId 사용하여 해결

이 문제를 해결하기 위해 WAS 서버마다 서로 다른 groupId를 사용하도록 변경했습니다.

- was1 → groupId = prd-was1
- was2 → groupId = prd-was2

이렇게 설정하면 각 WAS는 토픽의 모든 메시지를 각각 소비할 수 있어서 실시간 이벤트를 정상적으로 수신할 수 있게 되었습니다.
...
### 마무리하며
Kafka를 도입하면서 전자칠판 서비스의 실시간 이벤트 처리를 단순한 WebSocket 전송이 아닌 확장성과 안정성을 고려한 메시징 구조로 재설계할 수 있었습니다.

기능 단위 토픽 분리, 파티션 기반 병렬 처리 그리고 Consumer Group 설계를 통해 멀티 WAS 환경에서도 **실시간 이벤트를 안정적으로 전달**하고 있습니다.
...
kafka
kafka
](./img/kafka/kafka15.png)

또한 **Kafka 도입 시 중요한 부분 중 하나는 Lag이 얼마나 쌓이는지 모니터링하는 것**입니다.   

Lag란 Kafka에 미처리된 이벤트가 얼마나 남았는지에 정보를 가지고있는데 테스트 과정에서 적절한 Consumer 개수와 Partition 개수를 정할 때 Lag을 보면서 설계해야 합니다.
