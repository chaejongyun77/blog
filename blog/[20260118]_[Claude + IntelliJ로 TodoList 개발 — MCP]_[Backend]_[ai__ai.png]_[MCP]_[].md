...
마침 제가 주로 사용하는 **IntelliJ와 연동**할 수 있었고 요즘 많이 언급되고 있는 **MCP도 직접 써보면서** 경험을 쌓고 싶었습니다.
...
이 조합으로 간단한 **ToDoList 페이지를 구현하면서 Claude Code를 실제 개발 흐름 속에서 사용해보고 MCP가 어떤 역할을 하는지**도 함께 살펴보겠습니다.
...
## LLM & Agent
**MCP**를 이해하려면 먼저 **LLM과 Agent**에 대해서 이해해야 합니다.

![
claude
](./img/mcp/mcp1.png)

**LLM (Large Language Model)**은 기본적으로 학습 당시에 습득된 지식을 통해서 질문에 대해서 답변을 합니다.

즉, 학습되지 않은 정보에 대해서는 답변을 할 수 없고 질문에 대한 답변 이외에 음악을 튼다가나 컴퓨터내에서 파일을 검색한다던가의 추가적인 행동을 할 수 없습니다.
이러한 **LLM의 한계를 극복**하고 LLM 애플리케이션의 기능을 확장하기 위해서 우리는 LLM이 다른 애플리케이션이나 데이터 소스와 상호 작용을 하도록 할 수 있습니다.

예를 들어서 이번달의 대학 입시 정보가 필요하면 **구글 검색 엔진을 통해서 최신 자료를 검색**하여 그 자료를 기반으로 답변을 하도록 하거나 현재의 주가 정보를 알고 싶으면 **Yahoo Finance 웹사이트를 통해서 주가 정보를 얻어오도록** 할 수 있습니다. 

이렇게 **LLM과 연동되는 외부 애플리케이션이나 데이터 소스를 Tool**이라고 합니다.
그런데 Tool이 많을 경우 LLM은 주어진 문제에 대해서 어떤 Tool을 사용해야할지 생각을 하고 의사 결정을 해야 하는데 이렇게 **어떤 Tool을 사용할지 결정하고 Tool에서 얻은 정보를 통해서 답변을 만들어내는 역할을 하는 것이 바로 Agent**입니다.

## MCP 프로토콜의 동작 원리
MCP에 대해서 정확하게 이해하려면 **MCP는 정확하게 이야기 하면 Protocol**입니다. JSON-RPC/HTTP 를 이용하는 LLM 애플리케이션과 Tool 서버가 어떻게 통신하는지를 정의한 규약입니다.
**이 말은 이 규약만 따르면 어떤 SDK나 어떤 프로그래밍 언어등 사용할 수 있다는 것입니다.**
...
이번 실습을 통해 최근 유행하는 MCP의 개념을 아주 조금이나마 직접 체감해볼 수 있었고
Claude Code와 같은 AI 도구를 활용한 개발 방식이 얼마나 편리한지도 경험할 수 있었습니다. 

기존에는 모든 코드를 직접 타이핑해서 구현했다면
이제는 AI와 대화하듯 설계하고 수정하면서 마치 동료와 함께 작업하는 느낌이 들었습니다.

다만 장기적인 관점에서는 몇 가지 고민도 함께 들었습니다.
AI가 생성한 코드의 유지보수성과 품질은 신뢰할 수 있는지...
그리고 무엇보다 **보안적인 측면**에서 괜찮을지에 대한 걱정이 남습니다.
프로젝트 구조와 데이터 흐름을 AI가 이해하고 설계하는 과정에서
그 정보들이 어떻게 다뤄지고 보호되는지에 대해서는 아직 명확하지 않은 부분도 있다고 느꼈습니다.

그럼에도 불구하고 AI 도구는 이미 개발 과정에서 무시할 수 없는 존재가 되었고
이를 어떻게 잘 활용하고 검증하며 책임 있게 사용하는지가 개발자의 역량이 될 것이라고 생각합니다.
앞으로도 Claude Code와 같은 AI 도구를 활용하여 성장하는 개발자가 되기 위해 노력할 것입니다.